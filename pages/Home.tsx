...
export const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const sessionKey = `profileChecked_${currentUser.id}`;
    if (sessionStorage.getItem(sessionKey) || profileCheckedRef.current) return;
    ...
    const checkProfile = async () => {
      try {
        ...
        // Mark as checked so it never re-runs this session
        sessionStorage.setItem(sessionKey, "1");
      } catch {
        // Silently fail — don't block home page
      }
    };
      ...
      {currentUser?.role === "CUSTOMER" && (
        <ProfileSetupModal
          ...
          onClose={() => {
            ...
            // Mark dismissed for this session so it won't re-appear on nav
            sessionStorage.setItem(`profileChecked_${currentUser.id}`, "1");
          }}
            ...
            <div className="grid md:hidden grid-cols-1 gap-4">
              {buildTiers.map((tier, i) => (
                <div
                  ...
                  className={`card-hover cursor-pointer relative rounded-2xl p-5 flex flex-col gap-3 transition-all duration-300 hover:shadow-lg ${tier.featured
