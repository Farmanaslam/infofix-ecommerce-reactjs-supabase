...
import { ShoppingCart, Star, Heart, ArrowRight, Search, SlidersHorizontal, Eye, ShoppingBag, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="pb-32 overflow-hidden bg-white">
      ...
      <section className="relative h-[450px] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[30%] h-[50%] bg-blue-100 rounded-full blur-[100px] animate-float opacity-50" style={{ animationDelay: '2s' }}></div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 w-full text-center space-y-8">
          ...
          <h1 className="text-5xl md:text-7xl font-black text-gray-900 tracking-tighter animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
            The Digital <span className="text-indigo-600">Collection.</span>
          </h1>
          <p className="text-gray-500 text-lg max-w-2xl mx-auto font-medium animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            Explore our meticulously selected range of hardware and lifestyle essentials designed for those who live on the cutting edge.
          </p>
          ...
          <div className="max-w-2xl mx-auto relative group animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            ...
            <div className="relative glass rounded-2xl flex items-center p-2 shadow-2xl shadow-gray-200/50">
              ...
              <input 
                ...
                className="bg-transparent border-none focus:ring-0 flex-1 px-4 py-3 text-sm font-semibold text-gray-900"
              />
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-12 border-b border-gray-100 pb-8">
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide w-full md:w-auto">
            <button 
              ...
              className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                selectedCategory === 'All' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
              }`}
            >
              All Drops
            </button>
            {CATEGORIES.map(cat => (
              <button 
                ...
                className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                  selectedCategory === cat ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="flex items-center gap-4 shrink-0">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{filteredProducts.length} Results</p>
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
            {filteredProducts.map((product, index) => (
              <div 
                ...
                style={{ animationDelay: `${index * 0.1}s`, opacity: isLoaded ? 1 : 0 }}
              >
                {/* Image Container */}
                ...
                <div className="space-y-2 px-2 transition-transform duration-500 group-hover:translate-x-1">
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] text-indigo-600 font-black uppercase tracking-[0.2em]">{product.category}</p>
                  ...
                  <div className="flex items-end justify-between pt-1">
                    <div className="flex flex-col">
                      <span className="text-xs text-gray-400 font-medium">Retail Price</span>
                      <p className="font-black text-2xl text-gray-900 tracking-tighter">${product.price.toFixed(2)}</p>
                    </div>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">{product.reviews} Reviews</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="py-32 flex flex-col items-center justify-center text-center animate-fade-in-up">
            ...
            <p className="text-gray-500 font-medium max-w-sm mb-8">Try adjusting your filters or search keywords to find what you're looking for.</p>
            <button 
        ...
        <div className="relative bg-indigo-600 rounded-[48px] p-12 md:p-24 overflow-hidden text-center text-white">
          ...
          <div className="relative z-10 space-y-6 max-w-3xl mx-auto">
            ...
            <p className="text-indigo-100 text-lg font-medium opacity-80">
              Get first access to limited drops, technical whitepapers, and exclusive community events.
            </p>
