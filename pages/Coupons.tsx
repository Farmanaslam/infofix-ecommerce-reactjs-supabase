...
import { Plus, Trash2, Tag, ToggleLeft, ToggleRight, Copy, Check, Search, X, Package } from "lucide-react";
...
export const AdminCoupons: React.FC = () => {
  ...
  const handleCreate = async () => {
    ...
    const { error: err } = await supabase.from("coupons").insert({
      ...
      // NULL = all products, array = specific products
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
        ...
        <div className="mt-6 space-y-2">
            ...
            <div className="flex flex-wrap gap-2 p-3 bg-indigo-50 border border-indigo-100 rounded-xl">
              {selectedProductIds.map(id => {
                ...
                return (
                  <span
                    key={id}
                    className="flex items-center gap-1.5 bg-white border border-indigo-200 text-indigo-700 text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm"
                  >
                    {p?.image_url && (
                      <img src={p.image_url} alt="" className="w-4 h-4 rounded object-cover" />
                    )}
                    {p?.name ?? `#${id}`}
                    <button
              ...
              <button
                ...
                className="text-xs text-slate-400 hover:text-red-500 font-semibold px-2 transition"
              >
                Clear all
              </button>
            </div>
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {showProductDropdown && (
              <div className="absolute top-full left-0 right-0 z-50 mt-1.5 bg-white border border-slate-100 rounded-2xl shadow-xl max-h-64 overflow-y-auto">
                  ...
                  filteredProducts.map(p => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 text-left transition hover:bg-indigo-50 ${
          ...
          <p className="text-xs text-slate-400 font-medium">
            {selectedProductIds.length === 0
              ...
              : `🎯 This coupon will only apply to ${selectedProductIds.length} selected product${selectedProductIds.length > 1 ? "s" : ""}.`}
          </p>
        </div>

        {error && (
          <p className="mt-4 text-sm text-red-600 font-semibold bg-red-50 px-4 py-2 rounded-xl">
            {error}
          </p>
        )}
        {success && (
          <p className="mt-4 text-sm text-emerald-600 font-semibold bg-emerald-50 px-4 py-2 rounded-xl">
            ✓ {success}
          </p>
        )}
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div className="shrink-0">
                      <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2 flex items-center gap-2">
                        <span className="font-black text-indigo-700 tracking-widest text-sm">
                          {coupon.code}
                        </span>
                        <button
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {!coupon.is_active && (
                          <span className="text-[10px] font-black bg-slate-100 text-slate-400 px-2 py-0.5 rounded-full uppercase">
                            Inactive
                          </span>
                        )}
                        {isExpired && (
                          <span className="text-[10px] font-black bg-red-100 text-red-500 px-2 py-0.5 rounded-full uppercase">
                            Expired
                          </span>
                        )}
                        {isMaxed && (
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex flex-wrap gap-4 text-sm text-slate-500 font-medium">
                        ...
                        {(coupon as any).min_order_amount > 0 && (
                          <span>Min ₹{(coupon as any).min_order_amount}</span>
                        )}
                        <span>
                      ...
                      {/* Product restriction display */}
                      {productNames && productNames.length > 0 ? (
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-[10px] font-black text-amber-600 bg-amber-50 border border-amber-100 px-2 py-0.5 rounded-full uppercase tracking-wide">
                            🎯 Product Specific
                          </span>
