...
export const Dashboard: React.FC = () => {
  ...
  const stats = [
    ...
    { label: 'Total Orders', value: `+${orders.length}`, change: '+12.5%', icon: ShoppingBag, trend: 'up' },
    { label: 'Active Customers', value: '2,350', change: '+180.1%', icon: Users, trend: 'up' },
    { label: 'Inventory Alert', value: `${products.filter(p => p.stock < 20).length}`, change: '-4.4%', icon: TrendingUp, trend: 'down' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white p-6 rounded-2xl border shadow-sm flex flex-col gap-4">
            ...
            {orders.slice(0, 5).map((order) => (
              <div key={order.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors border border-transparent hover:border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold truncate max-w-[120px]">{order.customerName}</p>
          ...
          <div className="mt-6 p-4 bg-indigo-600 rounded-xl text-white relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <p className="text-sm leading-relaxed opacity-90">
                Nexus AI suggests restocking <strong>Nebula Headphones</strong> as demand has spiked by 12% this morning.
              </p>
            </div>
