...
export const Settings: React.FC = () => {
  const { currentUser, branches, addBranch, updateBranch, deleteBranch } = useStore();
  const [activeTab, setActiveTab] = useState<'General' | 'Security' | 'AI' | 'Staff' | 'Branches'>('General');
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tighter">System Settings</h2>
          <p className="text-gray-500 font-medium">Configure global parameters and administrative permissions.</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={isSaving}
            ...
            <div className="space-y-6">
              <div className="bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm space-y-8">
                <div className="flex items-center gap-4 pb-6 border-b">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-gray-900">Public Business Identity</h3>
                    <p className="text-sm text-gray-400">This information will be displayed across the customer storefront.</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Legal Trading Name</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Support Email Address</label>
                    <input 
                      type="email" 
                  ...
                  <div className="space-y-2 md:col-span-2">
                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Main Office Address</label>
                    <textarea 
                      rows={2}
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Customer Helpline</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Currency Unit</label>
            ...
            <div className="space-y-6">
              <div className="bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-400">Manage physical store locations and contact points.</p>
                </div>
                <button 
              ...
              <div className="grid grid-cols-1 gap-4">
                {branches.map((branch) => (
                  <div key={branch.id} className="bg-white p-6 rounded-[24px] border border-gray-100 shadow-sm flex items-center justify-between group hover:border-indigo-200 transition-all">
                    <div className="flex items-center gap-6">
                      <img src={branch.image} alt="" className="w-16 h-16 rounded-2xl object-cover shadow-sm" />
                      <div>
                        <h4 className="font-bold text-gray-900">{branch.title}</h4>
                        <div className="flex items-center gap-3 text-xs text-gray-400 font-medium mt-1">
                          ...
                          <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {branch.hours}</span>
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-6">
              <div className="bg-indigo-900 p-10 rounded-[40px] text-white relative overflow-hidden">
                <div className="relative z-10 space-y-6">
                  ...
                  <h3 className="text-4xl font-black tracking-tighter">Gemini Intelligence Hub</h3>
                  <div className="flex flex-wrap gap-4">
                    <div className="bg-white/10 px-6 py-4 rounded-3xl backdrop-blur-md">
                      <p className="text-[10px] font-black text-indigo-300 uppercase mb-1">Active Model</p>
                      <p className="text-lg font-bold">gemini-3-flash-preview</p>
                    </div>
                    <div className="bg-white/10 px-6 py-4 rounded-3xl backdrop-blur-md">
                      <p className="text-[10px] font-black text-indigo-300 uppercase mb-1">API Status</p>
                      <div className="flex items-center gap-2 text-green-400 font-bold">
                        <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span> Authorized
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-8 rounded-[32px] border shadow-sm">
                  ...
                  <p className="text-sm text-gray-500 mb-6">Automatically generate high-converting copy for new inventory items.</p>
                ...
                <div className="bg-white p-8 rounded-[32px] border shadow-sm">
                  ...
                  <p className="text-sm text-gray-500 mb-6">Receive daily AI recommendations on restocks and sales trends.</p>
            ...
            <div className="bg-white rounded-[32px] border shadow-sm overflow-hidden">
              <div className="p-8 border-b flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-400">Manage portal access for your team members.</p>
                </div>
                ...
                {OPERATORS.map((op) => (
                  <div key={op.id} className="p-6 flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-4">
                      <img src={op.avatar} className="w-12 h-12 rounded-2xl object-cover border-2 border-white shadow-sm" />
                      <div>
                    ...
                    <div className="flex items-center gap-8">
                      <div className="text-right">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                          op.role === 'MANAGER' ? 'bg-purple-100 text-purple-600' : 
                          op.role === 'INVENTORY' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'
                        }`}>
                          {op.role}
                        </span>
                        <p className="text-[10px] font-bold text-green-500 mt-1 uppercase">Logged In</p>
                      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden animate-fade-in-up">
            <div className="p-8 border-b flex items-center justify-between bg-gray-50">
              <div>
                <h3 className="text-2xl font-black text-gray-900">{editingBranch ? 'Edit Branch' : 'Add New Branch'}</h3>
                <p className="text-sm text-gray-500">Configure regional storefront details.</p>
              </div>
              <button 
            ...
            <form onSubmit={handleBranchSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Branch Title</label>
                  <input 
                    required
                    type="text" 
                    value={branchFormData.title}
                    onChange={(e) => setBranchFormData({...branchFormData, title: e.target.value})}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Operating Days</label>
                  <input 
                    required
                    type="text" 
                    value={branchFormData.days}
                    onChange={(e) => setBranchFormData({...branchFormData, days: e.target.value})}
                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-600 font-medium"
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    onChange={(e) => setBranchFormData({...branchFormData, hours: e.target.value})}
                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-600 font-medium"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    onChange={(e) => setBranchFormData({...branchFormData, mapsUrl: e.target.value})}
                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-600 font-medium"
              ...
              <div className="space-y-2">
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Branch Image URL</label>
                <div className="flex gap-4">
                  <input 
                    type="text" 
                    value={branchFormData.image}
                    onChange={(e) => setBranchFormData({...branchFormData, image: e.target.value})}
                  ...
                  <div className="w-12 h-12 rounded-xl bg-gray-100 overflow-hidden shrink-0 border">
                    <img src={branchFormData.image} alt="Preview" className="w-full h-full object-cover" />
                  </div>
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Additional Details</label>
                <textarea 
                  rows={3}
                  value={branchFormData.details}
                  onChange={(e) => setBranchFormData({...branchFormData, details: e.target.value})}
                  className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-600 font-medium resize-none"
