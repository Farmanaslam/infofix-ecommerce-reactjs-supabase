...
export const CustomerLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { cart, switchRole, currentPage, setCurrentPage } = useStore();
        ...
        <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="hidden md:block text-xs font-bold px-4 py-2 bg-gray-900 text-white rounded-xl hover:bg-indigo-600 transition-all shadow-lg shadow-gray-200"
            >
              Staff Portal
            </button>
      ...
      <footer className="bg-[#172337] text-white pt-16 pb-4">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 mb-16">
          <div className="space-y-4">
            <h4 className="text-sm font-bold uppercase tracking-wider text-gray-300">About</h4>
            ...
            <ul className="space-y-2 text-xs text-gray-400">
              ...
              <li><button onClick={() => setCurrentPage('branches')} className="hover:text-white transition-colors">Our Branches</button></li>
              <li><button onClick={() => setCurrentPage('updates')} className="hover:text-white transition-colors">Our Blog</button></li>
            </ul>
          </div>
          <div className="space-y-4">
            <h4 className="text-sm font-bold uppercase tracking-wider text-gray-300">Help</h4>
            <div className="w-6 h-[1px] bg-gray-500 mb-4"></div>
            <ul className="space-y-2 text-xs text-gray-400">
              <li><button className="hover:text-white transition-colors">Payments</button></li>
              <li><button className="hover:text-white transition-colors">Cancellation & Returns</button></li>
              <li><button className="hover:text-white transition-colors">FAQ</button></li>
              <li><button className="hover:text-white transition-colors">Raise Your Query</button></li>
              <li className="flex items-center gap-1">
                <Download className="w-3 h-3 text-orange-500" />
                <button className="hover:text-white transition-colors">Download Apk</button>
              </li>
            </ul>
          </div>
          <div className="space-y-4">
            <h4 className="text-sm font-bold uppercase tracking-wider text-gray-300">Policy</h4>
            <div className="w-6 h-[1px] bg-gray-500 mb-4"></div>
            <ul className="space-y-2 text-xs text-gray-400">
              ...
              <li><button className="hover:text-white transition-colors">Privacy & Security</button></li>
              <li><button className="hover:text-white transition-colors">Shipping</button></li>
            </ul>
          </div>
          <div className="space-y-4">
            <h4 className="text-sm font-bold uppercase tracking-wider text-gray-300">Social</h4>
            <div className="w-6 h-[1px] bg-gray-500 mb-4"></div>
            <ul className="space-y-2 text-xs text-gray-400">
              <li className="flex items-center gap-2"><Facebook className="w-3 h-3" /> <button className="hover:text-white transition-colors">Facebook</button></li>
              <li className="flex items-center gap-2"><Twitter className="w-3 h-3" /> <button className="hover:text-white transition-colors">Twitter</button></li>
              <li className="flex items-center gap-2"><Youtube className="w-3 h-3" /> <button className="hover:text-white transition-colors">YouTube</button></li>
              <li className="flex items-center gap-2"><Instagram className="w-3 h-3" /> <button className="hover:text-white transition-colors">Instagram</button></li>
              <li className="flex items-center gap-2"><Linkedin className="w-3 h-3" /> <button className="hover:text-white transition-colors">Linkedin</button></li>
            </ul>
          </div>
          <div className="space-y-4 lg:col-span-1">
            <h4 className="text-sm font-bold uppercase tracking-wider text-gray-300">Mail Us:</h4>
            <div className="w-6 h-[1px] bg-gray-500 mb-4"></div>
            <p className="text-xs text-gray-400 break-all">infofixcomputers1@gmail.com</p>
          </div>
          <div className="space-y-4 lg:col-span-1">
            <h4 className="text-sm font-bold uppercase tracking-wider text-gray-300">Office Address</h4>
            <div className="w-6 h-[1px] bg-gray-500 mb-4"></div>
            <div className="text-[11px] text-gray-400 leading-relaxed space-y-4">
              <p>Address: Ananda Gopal Mukherjee Sarani Rd, near BINA GAS, Kamalpur Plot, Benacity, Durgapur, West Bengal 713213</p>
        ...
        <div className="border-t border-gray-700/50 pt-8 pb-4">
          <div className="max-w-7xl mx-auto px-4 flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <div className="text-[11px] text-gray-400 font-medium">© {new Date().getFullYear()} Infofix Computers</div>
            <div className="flex flex-wrap items-center justify-center gap-2">
              {['VISA', 'MASTERCARD', 'MAESTRO', 'AMEX', 'DINERS', 'DISCOVER', 'RUPAY', 'NETBANKING', 'COD', 'EMI'].map(method => (
                <div key={method} className="h-6 px-2 bg-white/5 border border-white/10 rounded flex items-center justify-center text-[8px] font-black text-gray-400 grayscale hover:grayscale-0 transition-all cursor-default">{method}</div>
              ))}
            </div>
          </div>
        </div>
...
export const AdminLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const visibleNav = navItems.filter(item => item.role.includes(currentUser.role));
      ...
      <aside className="w-72 bg-white border-r flex flex-col h-screen sticky top-0 shadow-sm">
        <div className="p-8 border-b">
          <div className="flex items-center gap-3 mb-8">
            ...
            <span className="font-black text-xl tracking-tight">NexusAdmin</span>
          </div>
          
          <div className="p-4 bg-gray-50 rounded-2xl flex items-center gap-3 border border-gray-100">
            <img src={currentUser.avatar} alt={currentUser.name} className="w-10 h-10 rounded-xl shadow-sm" />
            <div className="overflow-hidden">
              <p className="text-xs font-bold text-gray-900 truncate">{currentUser.name}</p>
              <p className="text-[10px] text-indigo-600 font-bold uppercase tracking-widest">{currentUser.role}</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 p-6 space-y-2">
          {visibleNav.map((item) => (
            ...
            >
              <item.icon className={`w-5 h-5 transition-transform ${adminPage === item.name ? '' : 'group-hover:scale-110'}`} />
              {item.name}
            </button>
          ))}
        </nav>
        ...
        <header className="h-20 bg-white border-b flex items-center justify-between px-10">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <img src={currentUser.avatar} alt="" className="w-10 h-10 rounded-full border-2 border-indigo-100" />
            </div>
          </div>
        </header>
